...
function activateCheckpoint(pt) {
  ...
  if (isNewCheckpoint || checkpointCooldown === 0) {
    ...
    addBurst(pt.x, pt.y - 22, "#9be69d", 18);
    addPopup("HP vol", pt.x, pt.y - 68, "#b7ff9c");
...
function updateEffects() {
  checkpoints.forEach(pt => {
    if (pt.justActivated > 0) pt.justActivated--;
  });
...
function drawScene() {
  ...
  if (currentLevel===2) {
    drawParallaxImg(mistBackImg,      0, height, 0.08, 200, 160, 120, 200);
    drawParallaxImg(mistBackTreesImg, 0, height, 0.13, 70, 140, 100, 180);
    drawParallaxImg(mistTreeImg,      0, height, 0.21, 90, 120, 80, 160);
  } else {
    drawParallaxImg(mistBackImg,      0, height, 0.08, 255);
    drawParallaxImg(mistBackTreesImg, 0, height, 0.13, 95);
    drawParallaxImg(mistTreeImg,      0, height, 0.21, 145);
  }
  noTint();
  drawAtmosphere();
  camera.on();
...
// Draws a repeated background layer with optional color tint.
function drawParallaxImg(img, y, drawH, speed, alpha, r=255, g=255, b=255) {
  if (!img) return;
...
function drawCheckpoints() {
  checkpoints.forEach(pt=>{
    const glow = pt.activated ? 28 + sin(frameCount * 0.12) * 10 : 0;
    const pop = pt.justActivated > 0 ? sin(frameCount * 0.45) * 5 : 0;
    const poleColor = pt.activated ? "#143b2a" : "#10140f";
    const flagColor = pt.activated ? "#8cff8f" : "#ffd86b";
    const flagLight = pt.activated ? "#ddffd7" : "#fff3b0";

    if (pt.activated) {
      noStroke();
      fill(120, 255, 140, glow);
      ellipse(pt.x + 26, pt.y - 20, 86 + pop, 74 + pop);
    }

    stroke(poleColor); strokeWeight(7);
    line(pt.x,pt.y+45,pt.x,pt.y-36);
    strokeWeight(5); line(pt.x,pt.y-28,pt.x+34,pt.y-18);
    noStroke();
    fill(flagColor); rect(pt.x+21,pt.y-35-pop,28,30,5);
    fill(flagLight); rect(pt.x+27,pt.y-28-pop,13,18,2);
  });
  strokeWeight(1);
}
...
async function startTeachableMachineAudio() {
  ...
  try {
    ...
    setTmStatus("Audio actief", "Luister naar: roep (aaa), klap, stil");
  } catch (error) {
...
function handleAudioPrediction(labels, scores) {
  ...
  setTmStatus("Audio actief", `${formatAudioLabel(top.label)}: ${Math.round(top.score * 100)}%`);

  if (top.score < TM_CONFIDENCE) return;
  if (isRestClass(tmLastClass)) return;
  const isRoep = isRoepClass(tmLastClass);
  const isKlap = isClapClass(tmLastClass);
  ...
  if (isRoep) {
    ...
    ui.toast.textContent = "roep (aaa): spring vooruit";
    if (player) {
...
function isRoepClass(label) {
  return label === "roep" || label === "aaa" || label === "aaah" || label.includes("roep");
}

function isClapClass(label) {
  return label === "klap" || label === "clap" || label.includes("klap") || label.includes("clap");
}

function formatAudioLabel(label) {
  const clean = String(label || "").trim();
  const normalized = normalizeClassName(clean);
  if (isRoepClass(normalized)) return "roep (aaa)";
  if (isClapClass(normalized)) return "klap";
...
function updateHud() {
  ...
  if (ui.level) ui.level.textContent=currentLevel;
